...
static int llex(LexState *ls, TValue *tv)
{
  ...
  for (;;) {
    ...
    case '-':
      next(ls);
      if (ls->current == '=') {next(ls); return TK_subto;}
	...
	case '!':
	  next(ls);
      if (ls->current != '=') return TK_not; else { next(ls); return TK_ne; }
	case '&':
	  next(ls);
      if (ls->current != '&') return '&'; else { next(ls); return TK_and; }
	case '|':
	  next(ls);
      if (ls->current != '|') return '|'; else { next(ls); return TK_or; }
    case ':':
      next(ls);
	...
	case '+':
	  next(ls);
		if (ls->current != '=') return '+'; else { next(ls); return TK_addto; }
	case '*':
	  next(ls);
		if (ls->current != '=') return '*'; else { next(ls); return TK_multo; }
	case '/':
	  next(ls);
		if (ls->current != '=') return '/'; else { next(ls); return TK_divto; }
	case '%':
	  next(ls);
		if (ls->current != '=') return '%'; else { next(ls); return TK_modto; }
    case '.':
      save_and_next(ls);
      if (ls->current == '.') {
	next(ls);
...
void lj_lex_savelastp(LexState *ls){
#define SET(A) ls->lastp.A = ls->A
  SET(tokenval);
  SET(lookaheadval);
  SET(current);
  SET(token);
...
void lj_lex_swaplastp(LexState *ls){
#define SWAP(A) (A)=ls->lastp.A,ls->lastp.A=ls->A,ls->A=(A)
  TValue tokenval;
  TValue lookaheadval;
  int current;
  LexToken token;
